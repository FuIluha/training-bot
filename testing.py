...
import aiosqlite
from start import QuestionStates
...
@testing_router.message(F.text == 'С предупреждением ознакомлен', QuestionStates.waiting_after_agreement)
...
async def testing(message: Message, state: FSMContext):
    ...
    if question_index == 6:
        async with aiosqlite.connect('users.db') as db:
            ...
            await db.commit()
        await message.answer('Ваши результаты сохранены')
        await state.clear()
        return
